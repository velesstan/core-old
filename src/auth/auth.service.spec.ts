...
describe('Auth service', () => {
    ...
    it("should sign in and return access_token for valid user", async () => {
        await userService.create({ email: "admin@veles.services", password: "pass" })
        const token = await authService.signIn({ email: "admin@veles.services", password: "pass" })
        expect(token).not.toBe(null);
    });

    it("should throw unauthorized error for invalid credentials", async () => {
        await expect(authService.signIn({ email: "invalid", password: "invalid" })).rejects.toThrow("Unauthorized");
    })
});
